...
def test_crc8():
    path = os.path.join(os.getenv("BUILD_TESTS_DIR", "build/tests"), "crc.so")
    ...
    crc = ctypes.c_uint8(lib_blob.crc8(data, len(data_str), 0xFFFFFFFF))
    assert 0x0 != crc, "CRC most likely should NOT be 0x00"
    data_str = data_str + crc
    data = ctypes.cast(data_str, ctypes.POINTER(ctypes.c_uint8))
    crc = lib_blob.crc8(data, len(data_str), 0xFFFFFFFF)
